...
class Fase():
    ...
    def status(self):
        ...
        :return:
        """
        if not self._possui_porco_ativo(): return VITORIA
        elif self._pussir_passaros_ativos(): return EM_ANDAMENTO
        else: return DERROTA
    ...
    def _possui_porco_ativo(self):
        for porco in self._porcos:
            if porco.status == ATIVO: return True
        return False

    def _pussir_passaros_ativos(self):
        for passaro in self._passaros:
            if passaro.status == ATIVO: return True
        return False
