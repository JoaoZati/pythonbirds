...
class Ator():
    ...
    def colidir(self, outro_ator, intervalo=1):
        ...
        if self.status == DESTRUIDO or outro_ator.status == DESTRUIDO: return

        delta_x = abs(self.x - outro_ator.x)
        delta_y = abs(self.y - outro_ator.y)
        if delta_x <= intervalo and delta_y <= intervalo: self.status = outro_ator.status = DESTRUIDO
...
class Passaro(Ator):
    ...
    def foi_lancado(self):
        ...
        :return: booleano
        """
        return not (self._tempo_de_lancamento is None or self._angulo_de_lancamento is None)
